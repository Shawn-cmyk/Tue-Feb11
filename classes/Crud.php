...
class CRUD extends Database
{
    ...
    {
        $sql = "SELECT * FROM resources "; // INNER JOIN courses ON resourses.Cou_Name = courses.Gen_Id";
        $result = $this->conn->query($sql);
    ...
    public function AddResourse($course, $pic, $video)
    {
        $sql = "INSERT INTO resources(Cou_Name, Res_Filename, Video)
                            VALUES('$course','$pic','$video' )";
        if ($this->conn->query($sql)) {
            return 1;
        } else {
            echo "Insertion failed. " . $this->conn->error;
